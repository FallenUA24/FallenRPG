...
GAMESETUP_TIME = 15
PRE_GAME_TIME = 10                    -- How long after people select their heroes should the horn blow and the game start?
...
FORCE_PICKED_HERO = "npc_dota_hero_creep"

-- Fill this table up with the required XP per level if you want to change it
XP_PER_LEVEL_TABLE = {}
XP_PER_LEVEL_TABLE[0] = 0
for i=1,9 do
  XP_PER_LEVEL_TABLE[i] = XP_PER_LEVEL_TABLE[i-1] + 30*(i%10)
end

for i=10,19 do
  XP_PER_LEVEL_TABLE[i] = XP_PER_LEVEL_TABLE[i-1] + 10*(i%10) + 300
end

for i=20,29 do
  XP_PER_LEVEL_TABLE[i] = XP_PER_LEVEL_TABLE[i-1] + 80*(i%10) + 400
end

for i=30,39 do
  XP_PER_LEVEL_TABLE[i] = XP_PER_LEVEL_TABLE[i-1] + 90*(i%10) + 1200
end

for i=40,49 do
  XP_PER_LEVEL_TABLE[i] = XP_PER_LEVEL_TABLE[i-1] + 150*(i%10) + 2100
end

for i=50,59 do
  XP_PER_LEVEL_TABLE[i] = XP_PER_LEVEL_TABLE[i-1] + 200*(i%10) + 3600
end

for i=60,69 do
  XP_PER_LEVEL_TABLE[i] = XP_PER_LEVEL_TABLE[i-1] + 250*(i%10) + 5600
end

for i=70,79 do
  XP_PER_LEVEL_TABLE[i] = XP_PER_LEVEL_TABLE[i-1] + 275*(i%10) + 8100
end
...
function GameSettings:CaptureGameMode()
	if mode == nil then
		...
		mode:SetCustomAttributeDerivedStatValue(DOTA_ATTRIBUTE_STRENGTH_DAMAGE,0.75)
    	mode:SetCustomAttributeDerivedStatValue(DOTA_ATTRIBUTE_AGILITY_DAMAGE,0.75)
    	mode:SetCustomAttributeDerivedStatValue(DOTA_ATTRIBUTE_INTELLIGENCE_DAMAGE,0.75)

    	mode:SetCustomAttributeDerivedStatValue(DOTA_ATTRIBUTE_STRENGTH_HP,1.50)
    	...
    	mode:SetCustomAttributeDerivedStatValue(DOTA_ATTRIBUTE_INTELLIGENCE_MANA,0.00)
